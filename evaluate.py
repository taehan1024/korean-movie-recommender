"""Evaluation benchmark for recommendation models.

Metrics: Precision@K, Recall@K, Hit@K, MRR, DCG@10, nDCG@10.
Supports per-relevance-level breakdown (remake/thematic/genre),
bootstrap confidence intervals, and grid search over hybrid weights.
"""
...
def tune_weights(
    ...
    log.info("=== Weight Tuning (Grid Search) ===")
    ...
    for t, g, k, c, y in iterproduct(text_vals, genre_vals, keyword_vals, cast_vals, year_vals):
        ...
        if n_combos % 50 == 0:
            log.info(f"  Tested {n_combos} combos, best DCG@10={best_dcg:.4f}")

    log.info(f"  Total combos tested: {n_combos}")
    log.info(f"  Best DCG@10: {best_dcg:.4f}")
    log.info(f"  Best weights: {json.dumps({k: round(v, 3) for k, v in best_weights.items()})}")
    log.info(f"  Hit@10={best_metrics['Hit@10']:.4f}  MRR={best_metrics['MRR']:.4f}  "
...
def main():
    ...
    log.info(f"Loaded {len(gold)} gold pairs, US={len(us_df)}, KR={len(kr_df)}")
    log.info(f"Features available: {sorted(features.keys())}")
    ...
    for model_name in models:
        ...
        # Log results
        log.info(f"  Hit@5={metrics['Hit@5']:.4f}  Hit@10={metrics['Hit@10']:.4f} (+/-{metrics['Hit@10_se']:.4f})")
        log.info(f"  MRR={metrics['MRR']:.4f} (+/-{metrics['MRR_se']:.4f})")
        log.info(f"  P@5={metrics['P@5']:.4f}  P@10={metrics['P@10']:.4f}")
        log.info(f"  R@5={metrics['R@5']:.4f}  R@10={metrics['R@10']:.4f}")
        log.info(f"  DCG@10={metrics['DCG@10']:.4f} (+/-{metrics['DCG@10_se']:.4f})  "
                 f"NDCG@10={metrics['NDCG@10']:.4f}")
        log.info(f"  (n={metrics['num_queries']})")

        # Per-relevance breakdown
        log.info("  Per-relevance Hit@10:")
        for rel, score in metrics["per_relevance_Hit@10"].items():
            log.info(f"    {rel}: {score:.4f}")

        # Per-genre breakdown
        log.info("  Per-genre Hit@10:")
        for genre, score in metrics["per_genre_Hit@10"].items():
            log.info(f"    {genre}: {score:.4f}")
    ...
    if len(all_metrics) > 1:
        ...
        log.info(comparison[["model", "Hit@5", "Hit@10", "MRR", "P@10", "R@10", "DCG@10", "NDCG@10"]].to_string(index=False))

        # Check acceptance criteria
        best = max(all_metrics, key=lambda m: m["DCG@10"])
        log.info(f"Best model by DCG@10: {best['model']}")
        targets = {"Hit@10": 0.35, "MRR": 0.10, "R@10": 0.25, "DCG@10": 0.50}
        for metric, target in targets.items():
            val = best[metric]
            status = "PASS" if val >= target else "BELOW"
            log.info(f"  {metric}: {val:.4f} vs target {target:.2f} [{status}]")
