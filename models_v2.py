"""V2 recommendation models: bug fixes + keyword/year scoring + updated weights.

Changes from v1 (models.py):
- Fixed operator precedence bug in explain_recommendation (lines 166/169)
- Fixed cast_score asymmetry in explanation (now uses Jaccard, matching scorer)
- Added keyword Jaccard scoring
- Added year proximity scoring (Gaussian decay)
- Updated default hybrid weights (text-heavy per ReasoningRec findings)
- Improved fuzzy title matching with difflib
"""
...
def load_all_features() -> dict:
    """Load all feature matrices into a dict (v1 + v2 features)."""
    features = {}
...
def _hybrid_scores(
    ...
    # Optional v2 components (graceful fallback if features not built yet)
...
# ---------------------------------------------------------------------------
# Title resolution (improved with difflib)
# ---------------------------------------------------------------------------
...
def explain_recommendation(
    ...
    # Shared cast/crew + Jaccard score (BUG FIX: was precision, now Jaccard)
    if "cast_us" in features and "cast_vocab" in features:
        ...
        # FIX: Jaccard instead of precision to match _cast_scores()
...
def get_recommendations(
    ...
    """High-level recommendation API (v2)."""
