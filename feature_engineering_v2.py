"""V2 feature engineering: adds keyword multi-hot and year arrays.

Changes from v1 (feature_engineering.py):
- Added build_keywords(): TMDB keyword multi-hot encoding (cross-catalog bridge)
- Added build_years(): year arrays for proximity scoring
- Reuses v1 builders for tfidf, embedding, genre, cast
"""
...
# ---------------------------------------------------------------------------
# Sentence embeddings (unchanged from v1)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Genre multi-hot (unchanged from v1)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Cast/crew multi-hot (unchanged from v1)
# ---------------------------------------------------------------------------
...
def build_keywords(us: pd.DataFrame, kr: pd.DataFrame) -> None:
    ...
    if "keywords" not in us.columns or "keywords" not in kr.columns:
        print("  SKIP: 'keywords' column not found in CSVs. Run data ingestion with keyword fetch first.")
        return
    ...
    if not cross_kw:
        print("  SKIP: no cross-catalog keywords found.")
        return
    ...
    print(f"  Keywords: {len(cross_kw)} cross-catalog, "
...
def build_years(us: pd.DataFrame, kr: pd.DataFrame) -> None:
    ...
    print(f"  Years: US={us_valid}/{len(us)} valid, KR={kr_valid}/{len(kr)} valid")
...
def main():
    parser = argparse.ArgumentParser(description="Build feature matrices (v2)")
    parser.add_argument(
